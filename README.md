...
- **💎 Insight Hero 상단 배너** (로그인 즉시 노출) — 매일 매출 기회 확인
  - 매니저/관리자 로그인 시 대시보드 최상단에 자동 렌더
  - `radial-gradient` 3중 오로라 + 텍스트 그라디언트 → "숨은 매출 1.81억원"
  - 바로 옆 CTA 2개: `📞 리콜 시작` / `💬 상담 관리 →` (원클릭 액션)
  - 조건: `hasData && isManager && hiddenRevenue >= 10만원` → 무의미한 노출 방지
  - **검증**(2026-04-21): 숨은 매출 1억 8,120만원 / 휴면 환자 11명 / 데이터 기반 자동 계산 ✅
