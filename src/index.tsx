...
/* ─── 예약 관리 API ─── */
// 예약 기록 목록
app.get('/api/protected/reservations', async (c) => {
  const user = c.get('user')!
  const from = c.req.query('from') || ''
  const to = c.req.query('to') || ''
  const search = c.req.query('search') || ''
  ...
  const rows = await c.env.DB.prepare(`SELECT * FROM reservation_records WHERE ${where} ORDER BY record_date DESC`).bind(...params).all()
  return c.json({ data: rows.results })
})

// 예약 기록 등록/수정
app.post('/api/protected/reservations', async (c) => {
  const user = c.get('user')!
  const body = await c.req.json()
  ...
  await c.env.DB.prepare(`INSERT INTO reservation_records (id,hospital_id,record_date,day_of_week,cancel_count,dentweb_cancel_count,fulfillment_rate,memo,created_by) VALUES (?,?,?,?,?,?,?,?,?)`)
    .bind(id, user.hospitalId, body.record_date||'', body.day_of_week||'', body.cancel_count||0, body.dentweb_cancel_count||0, body.fulfillment_rate||0, body.memo||'', user.id).run()
  return c.json({ success: true, id })
})

// 예약 통계 (MUST be before :id routes)
app.get('/api/protected/reservations/stats', async (c) => {
  const user = c.get('user')!
  const from = c.req.query('from') || ''
  const to = c.req.query('to') || ''
  let df = ''
...
app.put('/api/protected/reservations/:id', async (c) => {
  const user = c.get('user')!
  const id = c.req.param('id')
  const body = await c.req.json()
  await c.env.DB.prepare(`UPDATE reservation_records SET record_date=?,day_of_week=?,cancel_count=?,dentweb_cancel_count=?,fulfillment_rate=?,memo=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND hospital_id=?`)
    .bind(body.record_date||'', body.day_of_week||'', body.cancel_count||0, body.dentweb_cancel_count||0, body.fulfillment_rate||0, body.memo||'', id, user.hospitalId).run()
  return c.json({ success: true })
})

app.delete('/api/protected/reservations/:id', async (c) => {
  const user = c.get('user')!
  const id = c.req.param('id')
  await c.env.DB.prepare('DELETE FROM reservation_records WHERE id=? AND hospital_id=?').bind(id, user.hospitalId).run()
  return c.json({ success: true })
})

/* ─── 대기시간 관리 API ─── */
app.get('/api/protected/wait-times', async (c) => {
  const user = c.get('user')!
  const from = c.req.query('from') || ''
  const to = c.req.query('to') || ''
  let where = 'hospital_id=?'
  ...
  if (from) { where += ' AND record_date >= ?'; params.push(from) }
  if (to) { where += ' AND record_date <= ?'; params.push(to) }
  const rows = await c.env.DB.prepare(`SELECT * FROM wait_time_records WHERE ${where} ORDER BY record_date DESC`).bind(...params).all()
  return c.json({ data: rows.results })
})

app.post('/api/protected/wait-times', async (c) => {
  const user = c.get('user')!
  const body = await c.req.json()
  ...
  await c.env.DB.prepare(`INSERT INTO wait_time_records (id,hospital_id,record_date,day_of_week,total_wait_minutes,avg_wait_minutes,memo,created_by) VALUES (?,?,?,?,?,?,?,?)`)
    .bind(id, user.hospitalId, body.record_date||'', body.day_of_week||'', body.total_wait_minutes||0, body.avg_wait_minutes||0, body.memo||'', user.id).run()
  return c.json({ success: true, id })
})

// 대기시간 통계 (MUST be before :id routes)
app.get('/api/protected/wait-times/stats', async (c) => {
  const user = c.get('user')!
  const from = c.req.query('from') || ''
  const to = c.req.query('to') || ''
  let df = ''
  ...
  if (to) { df += ' AND record_date <= ?'; params.push(to) }
  const bw = 'hospital_id=?' + df
  const bwData = bw + ' AND avg_wait_minutes > 0'
...
app.put('/api/protected/wait-times/:id', async (c) => {
  const user = c.get('user')!
  const id = c.req.param('id')
  const body = await c.req.json()
  await c.env.DB.prepare(`UPDATE wait_time_records SET record_date=?,day_of_week=?,total_wait_minutes=?,avg_wait_minutes=?,memo=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND hospital_id=?`)
    .bind(body.record_date||'', body.day_of_week||'', body.total_wait_minutes||0, body.avg_wait_minutes||0, body.memo||'', id, user.hospitalId).run()
  return c.json({ success: true })
})

app.delete('/api/protected/wait-times/:id', async (c) => {
  const user = c.get('user')!
  const id = c.req.param('id')
  await c.env.DB.prepare('DELETE FROM wait_time_records WHERE id=? AND hospital_id=?').bind(id, user.hospitalId).run()
  return c.json({ success: true })
})

/* ─── 주차권 관리 API ─── */
app.get('/api/protected/parking', async (c) => {
  const user = c.get('user')!
  const from = c.req.query('from') || ''
  const to = c.req.query('to') || ''
  let where = 'hospital_id=?'
  ...
  if (from) { where += ' AND record_date >= ?'; params.push(from) }
  if (to) { where += ' AND record_date <= ?'; params.push(to) }
  const rows = await c.env.DB.prepare(`SELECT * FROM parking_records WHERE ${where} ORDER BY record_date DESC`).bind(...params).all()
  return c.json({ data: rows.results })
})

app.post('/api/protected/parking', async (c) => {
  const user = c.get('user')!
  const body = await c.req.json()
  ...
  await c.env.DB.prepare(`INSERT INTO parking_records (id,hospital_id,record_date,day_of_week,ticket_count,memo,created_by) VALUES (?,?,?,?,?,?,?)`)
    .bind(id, user.hospitalId, body.record_date||'', body.day_of_week||'', body.ticket_count||0, body.memo||'', user.id).run()
  return c.json({ success: true, id })
})

// 주차권 통계 (MUST be before :id routes)
app.get('/api/protected/parking/stats', async (c) => {
  const user = c.get('user')!
  const from = c.req.query('from') || ''
  const to = c.req.query('to') || ''
  let df = ''
  ...
  if (to) { df += ' AND record_date <= ?'; params.push(to) }
  const bw = 'hospital_id=?' + df
  const bwData = bw + ' AND ticket_count > 0'
  const [total, dow, monthly] = await Promise.all([
...
app.put('/api/protected/parking/:id', async (c) => {
  const user = c.get('user')!
  const id = c.req.param('id')
  const body = await c.req.json()
  await c.env.DB.prepare(`UPDATE parking_records SET record_date=?,day_of_week=?,ticket_count=?,memo=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND hospital_id=?`)
    .bind(body.record_date||'', body.day_of_week||'', body.ticket_count||0, body.memo||'', id, user.hospitalId).run()
  return c.json({ success: true })
})

app.delete('/api/protected/parking/:id', async (c) => {
  const user = c.get('user')!
  const id = c.req.param('id')
  await c.env.DB.prepare('DELETE FROM parking_records WHERE id=? AND hospital_id=?').bind(id, user.hospitalId).run()
  return c.json({ success: true })
})
...
<script src="/static/modules/reservations.js"><` + `/script>
<script src="/static/modules/wait-times.js"><` + `/script>
<script src="/static/modules/parking.js"><` + `/script>
